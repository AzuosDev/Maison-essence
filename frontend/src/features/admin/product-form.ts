 ...
 *
 * ## Por que existe um "rascunho" e não o próprio `AdminProduct`
 *
 * Porque a tela e o banco discordam sobre o que e um preço. No banco e um
 ...
 * um preço válido. Um modelo que só aceita números obrigaria a tela a
 * adivinhar o que fazer com os estados intermediários, e o jeito comum de
 * adivinhar e apagar o que a pessoa digitou.
 *
 * Então o rascunho guarda **texto** nos campos numericos, e a conversão
 * acontece uma vez, na saída, depois que a validação passou.
 *
 * ## A chave de linha não e o id
 *
 * Cada variante carrega uma `key` que existe só aqui dentro. Uma variante
 * nova não tem id até o servidor responder, e usar o índice do array como
 * chave do React faria a linha 2 herdar o estado da linha 3 quando a 1 fosse
 * removida — o campo de preço com o valor do vizinho, que e o tipo de
 * defeito que só aparece depois de publicado.
 *
 * ## Tudo aqui e função pura
 *
 * E a parte que erra em silêncio: um preço lido errado entra no banco um
...
export interface VariantDraft {
  /** Chave estável de linha. Existe só na tela; nunca viaja para a API. */
  key: string;
  /** Presente: variante que já existe. Ausente: o servidor vai cria-lá. */
  id?: string;
  sku: string;
  label: string;
  /** Como esta digitado: `199,90`. Vira centavos na saída. */
  price: string;
  /** O preço "de", riscado no card. Vazio tira o desconto. */
...
export interface ProductDraft {
  name: string;
  /** Só na criação. A edição não mexe no endereço — ver `UpdateProductInput`. */
  slug: string;
  description: string;
  brand: string;
  categoryIds: string[];
 ...
 *
 * Contador, e não `crypto.randomUUID()`: a chave nunca sai desta aba e não
 * precisa ser única no mundo — precisa ser única nesta lista e estável entre
...
/**
 * Um produto novo, já com uma linha de variante.
 *
 * Não e conveniência: produto sem variante não existe no domínio, e o
 * servidor recusa com 422. Abrir o formulário com a tabela de variantes vazia
 * ensinaria a dona a salvar e só então descobrir que falta algo.
 */
...
/** O cadastro salvo, aberto para edição. */
export function draftFromProduct(product: AdminProduct): ProductDraft {
  return {
    ...
    variants: product.variants.map((variant) => ({
      ...
      // `null` e "não há preço de comparação", e vira campo vazio — e não
 ...
 *
 * O que se repete e o trabalho: preço, preço de comparação, estoque e as duas
 * chaves. O que **não** se repete e a identidade — `id` e `sku` saem fora,
 * porque são únicos por variante e copia-los faria a linha nova sobrescrever
 * a original ao salvar.
 *
 * O `label` fica como esta, de propósito. Quem duplica esta prestes a
 * escrever "50 ml" onde estava "100 ml", e apagar o campo tiraria a
 * referência do que ela esta copiando. Duas linhas com o mesmo label são
 * barradas pela validação, então o esquecimento não chega ao servidor.
 */
export function duplicateVariant(variant: VariantDraft): VariantDraft {
  // O `id` e retirado, e não zerado: com `exactOptionalPropertyTypes`, um
  // campo opcional presente valendo `undefined` não e a mesma coisa que um
  // campo ausente — e e a ausência que faz o servidor criar uma variante nova.
...
/**
 * Move uma foto de posição.
 *
 * A ordem do array **e** a ordem de exibição, e a primeira posição e a capa —
 * não há campo separado para ela. Por isso arrastar a terceira foto para o
 * início e o gesto que define a capa, e por isso `setCover` abaixo e apenas
...
/** Promove a foto a capa, que e dizer: leva para a primeira posição. */
 ...
 *
 * Confere o que o servidor conferiria, e não mais do que isso: a tela que
 * inventa regra própria acaba recusando um cadastro que a API aceitaria, e
 * quem esta do outro lado não tem como saber qual das duas esta errada.
 *
 * As duas checagens que **não** são copia do servidor são as de repetido —
 ...
 * dois desfechos e o que a dona espera de duas linhas idênticas na frente
 * dela.
 */
export function validateDraft(draft: ProductDraft): DraftErrors {
  ...
  // primeira ocorrência ainda parece única.
  for (const variant of draft.variants) {
    const label = normalizeLabel(variant.label);
    const sku = variant.sku.trim().toUpperCase();
...
/**
 * O nome da variante como quem lê a vitrine o entende.
 *
 * Caixa e espaço não diferenciam variante nenhuma: "100ml", "100 ML" e
 * " 100 ml " são o mesmo frasco para quem compra, e duas linhas assim são
 * sempre a mesma duplicação esquecida — alguém copiou a linha e mudou só o
 * jeito de escrever. O espaço sai por inteiro, e não e apenas colapsado:
 * "100 ml" ao lado de "100ml" e o caso que esta checagem existe para pegar.
 *
 * O valor daqui só serve para comparar. O que e salvo continua sendo o que a
 * dona escreveu, com os espaços dela.
 *
 * Não vale para o SKU: lá a normalização e a do servidor — apara e
 * maiusculiza —, e um espaço no meio de um código de estoque e uma diferença
 * de verdade.
 */
...
/** Há algo impedindo o salvamento. */
 ...
 * O corpo do `POST`.
 *
 * O `slug` só entra quando foi escrito: omitido, o servidor o gera a partir
 ...
 * O corpo do `PATCH`.
 *
 * O array de variantes vai inteiro, e e essa a razão de a tela nunca mostrar
 * uma variante que ela não mandaria de volta: o que sumir daqui e removido ou
 * aposentado do lado de lá.
 */
...
function variantToInput(variant: VariantDraft): AdminVariantInput {
  ...
  return {
    // Os três campos opcionais só entram quando tem valor. O `sku` vazio faz
    ...
    // A validação já passou, então `centsFromInput` não devolve `null` aqui.
    // O `?? 0` existe para o tipo, e não para o caso.
