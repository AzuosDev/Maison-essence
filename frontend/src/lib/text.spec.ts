...
/**
 * A normalização de acentos.
 *
 * Existe um caso para cada coisa que a faixa `̀-ͯ` pode errar em
 * silêncio. E fácil escrever essa faixa de um jeito que continua compilando,
 * continua passando nos testes que só usam palavras sem acento, e devolve o
 * texto intocado — a busca simplesmente para de achar "Acqua" quando alguém
 * digita "acqua", e ninguém descobre até um cliente reclamar.
 */
...
test('cobre os acentos que aparecem em português', () => {
  // O lado direito e sem acento de propósito: e o resultado da dobra.
...
test('mantem o comprimento do texto original', () => {
  // E disto que dependem os índices do realce da busca: a copia sem acento
...
test('não come letra nenhuma no caminho', () => {
  // Uma faixa mal escrita — `[0300-036f]`, sem os escapes de código — remove
