 ...
 * cartões grandes, um do lado do outro, cada um com o que a escolha implica
 * escrito nele. Não são dois radios pequenos numa lista — a decisão muda o
 * resto do formulário e muda o total, e um alvo de toque de 20px não e o
 * tamanho de uma decisão dessas no celular.
 *
 * ## Retirada não pede endereço. Nenhum.
 *
 * E o critério de aceite, e vale nas três camadas: os campos somem da tela,
 * `fulfillmentSchema` para de exigi-los e o corpo de `POST /orders` sai sem
 * o objeto de endereço. Pedir bairro para quem vai buscar na loja e o tipo
 * de campo que faz a pessoa fechar a aba.
 *
 * O que aparece no lugar e o que ela precisa: o endereço **da loja**, o
 * horário e as instruções que a dona escreveu no painel.
 *
 ...
 * de `/delivery-cities`, que já traz o rótulo pronto — e e confirmado pela
 * cotação que sai no mesmo clique. Os dois números vem do mesmo servidor;
 * nenhum deles e calculado aqui.
 *
 * ## O endereço que a conta já tem
 *
 * Cliente logado com endereço salvo escolhe com um toque. E o único ganho
 * real de ter conta nesta loja, e ele acontece justamente no passo que mais
 * cansa. Sem conta, nada muda: o formulário simplesmente pergunta.
 */
...
export function StepFulfillment({
   ...
   * digitando a primeira letra da rua. Validar só no clique deixaria o erro
   * na tela enquanto a pessoa corrige o campo ao lado. O meio-termo e
   * clássico e e este: silêncio até a primeira tentativa, revalidação viva
   * dali em diante — que sai de graça por o resultado ser recalculado no
  ...
  /**
   * A cidade que este navegador já escolheu na página do produto.
   *
   * Preenchida uma vez, e só quando a loja ainda atende aquela cidade: uma
   * cidade removida do painel simplesmente não casa, e o seletor abre como
   * abre para quem nunca escolheu. Depois que há escolha no checkout, o
   * efeito não tem mais o que fazer.
   */
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* A retirada só aparece quando a loja a oferece. Escondida, e um
 ...
 *
 * Tudo sai do painel. Sem endereço cadastrado, o bloco diz a verdade — a
 * loja combina o ponto pelo WhatsApp — em vez de desenhar uma moldura vazia
 * com três linhas em branco.
 */
 ...
 *
 * Duas fontes, o mesmo servidor. A cotação e a que vale — e ela que conhece
 * o subtotal e sabe se o frete grátis foi atingido —, mas ela leva uma ida
 * de rede para responder. Enquanto isso, o rótulo que veio com a lista de
 * cidades já esta em mãos e diz a taxa cheia daquela cidade.
 *
 * Mostrar o rótulo primeiro e trocar pelo da cotação e o que faz o valor
 * aparecer **no mesmo clique** da escolha, sem número nenhum calculado aqui:
...
function DeliveryFee({
  ...
  // A cotação só responde por esta cidade quando foi ela que a pediu. Entre
  // o clique e a resposta, a cotação em mãos ainda e a da cidade anterior.
  const quoted =
    quote !== undefined &&
...
interface SavedAddressOption {
  ...
  /** Vazio quando a cidade do endereço salvo não e mais atendida. */
  cityId: string;
}

/**
 * Os endereços da conta, prontos para um clique.
 *
 * A cidade só acompanha quando a loja ainda a atende: um endereço salvo
 * quando ela entregava em Barbalha não pode selecionar uma cidade que sumiu
 * da lista. Nesse caso o endereço entra e a cidade continua a escolher — que
 * e melhor que não oferecer o endereço de jeito nenhum.
 */
...
/**
 * A loja ainda atende a cidade deste endereço salvo.
 *
 * `null` não casa com nada, e endereço salvo sem cidade vinculada existe: e
