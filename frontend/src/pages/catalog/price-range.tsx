...
/**
 * A faixa de preço: dois controles deslizantes sobre um trilho só.
 *
 * ## Por que dois `<input type="range">` nativos
 *
 * Um slider duplo desenhado a mão precisa reimplementar arrasto, toque,
 * teclado, foco e anuncio — e costuma errar em pelo menos um. Dois ranges
 * nativos empilhados entregam tudo isso de graça: seta anda um passo,
 * Home e End vão aos extremos, o leitor de tela anuncia o valor, e o alvo
 * de toque e o que o sistema operacional definiu.
 *
 * O preço e o empilhamento: o trilho e um só na tela, mas são dois
 * elementos. O truque esta no CSS — os dois inputs ficam transparentes e
 * sem altura de trilho, e só as bolinhas recebem eventos.
 *
 * ## Quando o filtro e aplicado
 *
 * No fim do gesto, e não durante. Arrastar dispara uma dezena de eventos por
 * segundo; aplicar em cada um significaria uma entrada no histórico do
 * navegador e uma requisição por pixel. O valor local acompanha o dedo, e a
 * viagem até a URL acontece quando o cliente solta — ou tira o foco, ou
 * larga a tecla.
 *
 * ## As pontas não se cruzam
 *
 * O mínimo para um passo antes do máximo, e vice-versa. Sem isso, arrastar a
 * ponta esquerda além da direita produz uma faixa invertida, que o backend
 * obedeceria respondendo zero produtos.
 */

/** O passo do slider, em reais. Centavo em filtro de vitrine e ruído. */
const STEP_REAIS = 10;
...
export interface PriceRangeProps {
  ...
  /** O maior preço do catálogo neste contexto, em centavos. */
  ceilingCents: number;
  /** Chamado no fim do gesto, com a faixa já em ordem. */
...
export function PriceRange({ minCents, maxCents, ceilingCents, onCommit }: PriceRangeProps) {
  ...
  // — "limpar filtros", ou o botão de voltar do navegador. Ajustado durante
  // o render, comparando com o que já foi aplicado, e não por efeito: o
  ...
  const commit = (): void => {
    ...
    // ausência na URL em vez de `min=0` — o link fica mais curto e o backend
    ...
    <div className={styles.field}>
      ...
      <div className={styles.track} style={fill}>
        ...
        <input
          ...
          // O valor cru e um número solto; sem isto o leitor de tela anuncia
          // "cento e vinte" e não "cento e vinte reais".
...
/**
 * O teto do slider: o maior preço do catálogo, arredondado para cima.
 *
 ...
 * em R$ 1.840 — e o cliente veria o filtro esconder um produto que ele não
 * pediu para esconder.
 *
 * O mínimo de um passo existe para o catálogo vazio, em que o teto chega
