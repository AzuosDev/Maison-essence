...
/**
 * A última rede, acima do router.
 *
 * O `errorElement` de cada rota cobre o que acontece dentro da rota. O que
 * quebra fora dela — um provider, o próprio router — passaria direto e
 * deixaria a página em branco. Este boundary fica em volta de tudo.
 *
 * E uma classe porque só classe pode ser um error boundary: `componentDidCatch`
 * e `getDerivedStateFromError` não tem equivalente em hook, e não e esquecimento
...
export class AppErrorBoundary extends Component<AppErrorBoundaryProps, AppErrorBoundaryState> {
  ...
  override componentDidCatch(error: Error, info: ErrorInfo): void {
    // Sem serviço de monitoramento ainda: o console e o que há. Quando
    ...
    return (
      <MessageScreen
        ...
        actions={
          <Button
            onClick={() => {
              // Recarga completa, e não `setState({ error: null })`: o erro
              // veio de fora do router, e o estado que o causou continuaria
              // ali. Recomecar do zero e o único jeito honesto.
