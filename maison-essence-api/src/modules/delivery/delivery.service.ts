...
/** Campos que o PATCH substitui direto, sem regra propria. */
const PLAIN_FIELDS = ['name', 'state', 'feeCents', 'estimatedDays', 'isActive', 'order'] as const;

/** O que o calculo recebe: modo, cidade e o subtotal de produtos. */
export interface FeeQuery {
  mode: FulfillmentMode;
  /** Ausente ou `null` na retirada, onde nao ha cidade a escolher. */
  cityId?: string | null;
...
/**
 * A resposta do calculo, pronta para virar o snapshot do pedido.
 *
 * Alem da taxa, carrega o que o pedido precisa congelar: nome da cidade,
 * estado e prazo. E o que cumpre a promessa de que um pedido antigo continua
 * legivel depois de a cidade ser desativada ou reajustada — o pedido nao
 * guarda uma referencia que amanha responde outra coisa, guarda a copia do
 * que foi combinado.
 */
export interface DeliveryQuote extends ResolvedFee {
  ...
  /** `false` na retirada: quem vem buscar nao informa endereco. */
 ...
 *
 * `resolveFee` e a unica fonte da taxa no sistema inteiro. O modulo de
 * pedidos pergunta aqui e grava a resposta; ninguem mais multiplica, soma ou
 * compara valor de frete. A razao e a de sempre com dinheiro: duas
 * implementacoes da mesma regra divergem no dia em que uma delas e corrigida,
 * e a divergencia aparece na conta do cliente.
 */
@Injectable()
export class DeliveryService {
  ...
  /**
   * A lista do checkout: so as ativas, ordenadas, com a taxa escrita.
   *
   * Vai junto o `updatedAt` mais recente entre as cidades e as configuracoes,
   * porque o rotulo de frete gratis de uma cidade sem regra propria depende da
   * regra global da loja: mexer so em `StoreSettings` muda esta resposta, e o
   ...
   *
   * `minOrderForFreeCents` distingue ausente de `null`: ausente nao mexe na
   * regra, `null` apaga a regra propria e devolve a cidade a regra global.
   */
  ...
  /**
   * Regrava a posicao de todas as cidades citadas em uma operacao so.
   *
   ...
   * Exclui a cidade.
   *
   * Sem checagem de pedidos, ao contrario da exclusao de categoria: o pedido
   * guarda nome, estado, prazo e taxa em copia propria, e por isso continua
   * legivel depois que a cidade some. O que se perde e a possibilidade de
   * voltar a atender ali sem recadastrar — por isso o painel oferece
   * desativar, que e o caminho de quem so quer parar por um tempo.
   */
  ...
  /**
   * A taxa de entrega de um pedido. Fonte unica da verdade.
   *
   * Le a loja a cada chamada, e nao na inicializacao: a dona pode ligar a
   * retirada ou mudar o minimo do frete gratis no painel, e o proximo pedido
   * ja sai pela regra nova, sem redeploy.
   */
  async resolveFee(query: FeeQuery): Promise<DeliveryQuote> {
    ...
    if (query.mode === FULFILLMENT_MODES.PICKUP) {
      ...
      // que ninguem combinou: a taxa zera, mas nao ha onde buscar.
   ...
   * mensagem: para quem esta comprando, a diferenca entre "nao atendemos ai" e
   * "paramos de atender ai" nao muda nada — o que ele precisa saber e que essa
   * escolha nao vale mais e que a retirada continua de pe.
   */
  ...
  /** Busca pelo id, tratando id malformado como "nao encontrada". */
  ...
  /** Salva traduzindo a colisao de cidade repetida em 409. */
...
/** A alteracao mais recente entre as cidades e as configuracoes da loja. */
