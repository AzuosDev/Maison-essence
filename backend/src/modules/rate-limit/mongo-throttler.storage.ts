 ...
 * cair em instâncias diferentes — ou seja, de nenhum esforço.
 *
 * O Mongo já esta na frente de qualquer request desta API, então o contador
 * compartilhado sai sem serviço novo para manter. Redis seria mais rápido por
 * chamada, e e a troca a fazer se o volume justificar: a única coisa a
 * reescrever e esta classe.
 *
 * Janela fixa, e não deslizante: a primeira chamada abre a janela e ela vale
 * pelo tempo declarado. Janela fixa deixa passar até o dobro do limite na
 * virada — trinta no fim de um minuto e trinta no começo do seguinte —, e isso
...
export class MongoThrottlerStorage implements ThrottlerStorage {
   ...
   * Mongo ainda não varreu — nos dois casos a janela nasce em 1, e não somada
   * a de antes.
   */
  private async openWindow(key: string, now: number, ttl: number): Promise<OpenWindow> {
    ...
    try {
      ...
      // A outra invocação venceu a corrida do `upsert`: a janela dela já vale,
      // e esta chamada só precisa ser contada dentro dela.
