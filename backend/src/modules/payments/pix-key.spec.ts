...
describe('normalizePixKey', () => {
  it('aceita vazio: e a loja que ainda não configurou a chave', () => {
    expect(normalizePixKey('', PIX_KEY_TYPES.EMAIL)).toBe('');
    expect(normalizePixKey('   ', PIX_KEY_TYPES.CPF)).toBe('');
  });

  describe('CPF e CNPJ', () => {
    it('tira a pontuação que vem colada do aplicativo do banco', () => {
    ...
    it('recusa quantidade de digitos que não e a do documento', () => {
  ...
  describe('telefone', () => {
    ...
    it('deixa passar quem já veio com o pais', () => {
  ...
  describe('chave aleatória', () => {
    it('aceita o código que o banco gera', () => {
