...
/**
 * O telefone do cliente, em um lugar só.
 *
 ...
 * criada depois. Guardado torto, não da erro nenhum — da uma conversa que
 * nunca começa.
 *
 * Por isso a regra faz as duas coisas, como a do número da loja. Normaliza o
 ...
 * e recusa o que não e celular brasileiro com DDD.
 */

/** Guardado sempre assim: DDD mais nove digitos, sem o código do pais. */
export const PHONE_PATTERN = /^\d{11}$/;

/**
 * Celular brasileiro: DDD de 11 a 99 e o nono digito.
 *
 * Nenhum DDD tem zero, nem na frente nem atrás — por isso os dois primeiros
 * digitos são de 1 a 9. O terceiro digito e o `9` que toda operadora passou a
 * exigir em celular: fixo de oito digitos não entra, e não e descuido — a
 * mensagem do pedido vai para o WhatsApp, e o número que não recebe WhatsApp
 * não serve ao único canal de atendimento que a loja tem.
 */
const BRAZILIAN_MOBILE = /^[1-9][1-9]9\d{8}$/;

const COUNTRY_CODE = '55';

/** O que o cliente cola junto do número e que não e digito. */
 ...
 *
 * O `55` da frente e removido quando o resto tem cara de número brasileiro:
 * quem digita o próprio número raramente inclui o pais, mas quem cola de um
 ...
 *
 * Quando a normalização falha o valor original e devolvido — e não uma string
 * vazia — para que o erro fale do que o cliente digitou, com a mensagem
 * mostrando o formato, em vez de um "campo obrigatório" que não explica nada.
 */
