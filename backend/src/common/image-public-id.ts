...
/**
 * A identidade de uma imagem no sistema, em um lugar só.
 *
 * O banco guarda o `publicId` do Cloudinary e nunca a URL completa: trocar de
 * conta, de CDN ou de transformação depois não exige migrar dado nenhum — a
 * URL e montada na leitura, por `cloudinary.url.ts`.
 *
 * O prefixo de pasta não e organização: e autorização. Todo `publicId` que
 * entra no banco precisa estar dentro de uma das pastas abaixo, e e a
 * assinatura de upload que garante que só essas pastas podem ser escritas.
 ...
 *
 * Por isso a regra mora em `common/` e não no módulo de uploads: quem grava
...
/** As únicas pastas em que o painel pode escrever. */
 ...
 *
 * O conjunto de caracteres e o mesmo do slug — minúsculas, digitos e hífen —
 * porque o `publicId` vira caminho de URL. Ponto e barra ficam de fora de
 * propósito: são eles que permitiriam um `../` subir para fora da pasta.
 */
...
/** Mesma regra, aceitando vazio: campo de imagem única sem foto escolhida. */
...
/** De qual pasta a imagem e, ou `null` quando o `publicId` não e da loja. */
...
/** Valida um campo de imagem obrigatório. `each: true` para array de fotos. */
