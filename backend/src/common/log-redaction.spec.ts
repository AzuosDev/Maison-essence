...
describe('maskPhone', () => {
  it('guarda o DDD e os dois últimos digitos', () => {
    expect(maskPhone('88999991234')).toBe('88*******34');
  });

  it('ignora a pontuação do número digitado', () => {
    expect(maskPhone('(88) 99999-1234')).toBe('88*******34');
  });

  it('some inteiro com o que não chega a ser um telefone', () => {
...
describe('redactSensitive', () => {
  ...
  it('esconde a senha que alguém colocou no objeto logado', () => {
    expect(redactSensitive('{"email":"a@b.com","password":"senha-secreta-2026"}')).toBe(
      '{"email":"a@b.com","password":"[redigido]"}',
    );
  });

  it('esconde a credencial dentro da URI de conexão', () => {
    expect(
  ...
  it('esconde hash de sessão e chave de rate limit', () => {
  ...
  it('deixa em paz o que não e segredo', () => {
